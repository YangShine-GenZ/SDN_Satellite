...
def Creating_All_Access():
    ...
    for each_sat in sat_list:
        ...
        # 发射机与接收机相连
        # 与后面的卫星的接收机相连
        B_Name = sat_dic['node_' + str(now_plane_num) + '-' + str((now_sat_num + 1) % 12)].InstanceName
        F_Name = sat_dic['node_' + str(now_plane_num) + '-' + str((now_sat_num - 1) % 12)].InstanceName
        ...
        access_backward = now_sat_transmitter.GetAccessToObject(
            Get_sat_receiver(sat_dic['node_' + str(now_plane_num) + '-' + str((now_sat_num + 1) % 12)]))
        # 与前面的卫星的接收机相连
...
# 如果不是读取当前场景，即首次创建场景
if not Read_Scenario:
    Creat_satellite(numOrbitPlanes=6, numSatsPerPlane=12, hight=850, Inclination=85)  # Starlink
    #Creat_satellite(numOrbitPlanes=3, numSatsPerPlane=9, hight=11000, Inclination=65)
    #Creat_satellite(numOrbitPlanes=3, numSatsPerPlane=1, hight=36000, Inclination=0)
    # Kuiper
...
for i in range(0,10):
    ...
    response = requests.post('http://192.168.56.146:8000/modify/', data = data_list,headers=headers)
    print("--- topo uptade ---")
    data_list = []
    if i==0:
        mid_link()
