...
def Creating_All_Access():
    ...
    for each_sat in sat_list:
        ...
        if(now_plane_name == 'LEO_'):
            ...
            numSatsPerPlane = 1
            print(now_sat_name)
            #B_Name = sat_dic['GEO_' + str((now_plane_num+1)%numOrbitPlanes) + '-' + '0'].InstanceName
            #F_Name = sat_dic['GEO_' + str((now_plane_num-1)%numOrbitPlanes) + '-' + '0'].InstanceName
            #L_Name = sat_dic['MEO_' + str((now_plane_num - 1) % numOrbitPlanes) + '-' + str(now_sat_num)].InstanceName
            #R_Name = sat_dic['MEO_' + str((now_plane_num + 1) % numOrbitPlanes) + '-' + str(now_sat_num)].InstanceName
            #access_backward = now_sat_transmitter.GetAccessToObject(
                #et_sat_receiver(sat_dic['GEO_' + str(now_plane_num) + '-' + str((now_sat_num + 1) % numSatsPerPlane)]))
            # 与前面的卫星的接收机相连
            #access_forward = now_sat_transmitter.GetAccessToObject(
            #    Get_sat_receiver(sat_dic['GEO_' + str(now_plane_num) + '-' + str((now_sat_num - 1) % numSatsPerPlane)]))
            # 与左面的卫星的接收机相连
            access_left = now_sat_transmitter.GetAccessToObject(
                Get_sat_receiver(sat_dic['MEO_' + str((now_plane_num) % numOrbitPlanes) + '-' + str(now_sat_num)]))
            #与右面的卫星的接收机相连
            access_right = now_sat_transmitter.GetAccessToObject(
                Get_sat_receiver(sat_dic['LEO_' + str((now_plane_num) % numOrbitPlanes) + '-' + str(now_sat_num)]))
            #B_Range = Compute_access(access_backward)
            #F_Range = Compute_access(access_forward)
            L_Range = Compute_access(access_left)
            #R_Range = Compute_access(access_right)
            #B_Dict = {'geo1': now_sat_name, 'geo2': B_Name, 'bw': 20, 'delay': str(int(B_Range[0] / 300)) + 'ms',
                      #'jitter': '1ms',
                      #'loss': 0}
            #F_Dict = {'geo1': now_sat_name, 'geo2': F_Name, 'bw': 20,
                      #'delay': str(int(F_Range[0] / 300)) + 'ms', 'jitter': '1ms', 'loss': 0}
            #_Dict = {'geo1': now_sat_name, 'meo2': L_Name, 'bw': 20, 'delay': str(int(L_Range[0] / 300))+'ms', 'jitter': '1ms',
                      # 'loss': 0}
            #R_Dict = {'geo1': now_sat_name, 'meo2': R_Name, 'bw': 20, 'delay':str(int(R_Range[0] / 300))+'ms', 'jitter': '1ms',
                       #'loss': 0}
            #data_list.append(B_Dict)
            #data_list.append(F_Dict)
            #data_list.append(L_Dict)
...
def Change_Sat_color(sat_list):
    ...
    for each_sat in sat_list:
        ...
        orbit.IsOrbitVisible = True  # 将轨道设置为不可见
...
# 如果不是读取当前场景，即首次创建场景
if not Read_Scenario:
    Creat_satellite(numOrbitPlanes=6, numSatsPerPlane=12, hight=850, Inclination=85,name='LEO_')  # Starlink
    Creat_satellite(numOrbitPlanes=3, numSatsPerPlane=9, hight=11000, Inclination=65,name='MEO_')
    Creat_satellite(numOrbitPlanes=3, numSatsPerPlane=1, hight=36000, Inclination=0,name='GEO_')
    # Kuiper
