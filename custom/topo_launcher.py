...
def generate_elephant_flows(id, duration, net, log_dir):
    ...
    end_points = random_check.sample(hosts, 2)
    src = net.get(str(end_points[0]))
    dst = net.get(str(end_points[1]))

    # select connection params
    protocol = random_check.choice(protocol_list)
    port_argument = str(random_check.randint(port_min, port_max))
    bandwidth_argument = random_check.choice(elephant_bandwidth_list)
...
def generate_mice_flows(id, duration, net, log_dir):
    ...
    protocol = random_check.choice(protocol_list)
    port_argument = str(random_check.randint(port_min, port_max))
    bandwidth_argument = random_check.choice(mice_bandwidth_list)
...
def generate_flows(n_elephant_flows, n_mice_flows, duration, net, log_dir):
    ...
    for i in range(n_mice_flows):
        flow_type.append('M')
    random_check.shuffle(flow_type)

    # setting random flow start times
    flow_start_time = []
    for i in range(n_total_flows):
        n = random_check.randint(1, interval)
        if i == 0:
